import { GAME_ID, HISTORY_LIMIT, STORAGE_KEY, clamp } from './config.js';
...
export async function loadArchiveFromServer() {
  ...
  try {
    const res = await fetch(`/api/archive?game=${encodeURIComponent(GAME_ID)}`, {
...
function pushArchiveToServer(archive) {
  ...
  fetch('/api/archive', {
    ...
    body: JSON.stringify({ game: GAME_ID, archive }),
    signal: AbortSignal.timeout(5000),
