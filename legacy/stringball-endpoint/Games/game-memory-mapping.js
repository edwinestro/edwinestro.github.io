export function createMemoryMappingGame({ mount, statusBar, controls, message }){
  ...
  const grid=document.createElement('div'); grid.style.cssText='position:relative;display:grid;grid-template-columns:repeat(3, clamp(60px, 20vw, 100px));gap:12px;z-index:2;padding:10px;'; board.appendChild(grid);
  const cells=[]; for(let i=0;i<state.size;i++){ const btn=document.createElement('button'); btn.className='tile'; btn.setAttribute('aria-label', 'Cell ' + (i+1)); btn.style.background=`linear-gradient(140deg, ${palette[i]}, #0b0f18 80%)`; btn.dataset.index=i; btn.addEventListener('click',()=> select(i,btn)); grid.appendChild(btn); cells.push(btn); }
  ...
  const levelChip = chip(`<strong>${T('level')}:</strong><span>0</span>`); statusBar.appendChild(levelChip);
  const remainChip = chip(`<strong>${T('remain')}:</strong><span>0</span>`); statusBar.appendChild(remainChip);
  const levelSpan = levelChip.querySelector('span');
  const remainSpan = remainChip.querySelector('span');
  ...
  function next(){ state.level++; levelSpan.textContent=state.level; let idx; do{ idx=Math.floor(Math.random()*state.size);} while(state.pattern[state.pattern.length-1]===idx); state.pattern.push(idx); state.userIdx=0; remainSpan.textContent=state.pattern.length; playSequence(); }
