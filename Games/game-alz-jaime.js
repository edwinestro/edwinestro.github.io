export function createAlzJaimeGame({ mount, statusBar, controls, message }){
  ...
  const tiles=[]; for(let i=0;i<state.pool;i++){ const t=document.createElement('button'); t.className='tile'; t.dataset.index=i; t.style.background=`linear-gradient(140deg, hsl(${(i*37)%360} 70% 55%), #10141f 80%)`; t.addEventListener('click',()=> select(i,t)); board.appendChild(t); tiles.push(t);} 
  ...
  statusBar.appendChild(chip('<strong>Stage:</strong><span id="ajLvl">0</span>'));
  statusBar.appendChild(chip('<strong>Progress:</strong><span id="ajProg">0/3</span>'));
  ...
  function next(){ state.level++; q('#ajLvl').textContent=state.level; newPattern(); state.cleared=0; q('#ajProg').textContent='0/3'; lightPattern(); state.active=true; message.textContent=text[state.lang].yourTurn; renderHint(); }
  ...
  function q(sel){ return mount.ownerDocument.querySelector(sel);} function btn(label,cls,fn){ const b=document.createElement('button'); b.textContent=label; b.className=cls; b.addEventListener('click',fn); return b;} function chip(html){ const d=document.createElement('div'); d.className='status-chip'; d.innerHTML=html; return d; }
  syncTexts();
  return { destroy(){ mount.innerHTML=''; hintBox.remove(); } };
}
