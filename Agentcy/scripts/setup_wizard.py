...
- optionally writes Agentcy/.env (do NOT commit secrets)
...
def _write_env(values: dict[str, str]) -> None:
    lines: list[str] = []
    lines.append("# Generated by Agentcy/scripts/setup_wizard.py")
...
def main() -> int:
    ...
    if not ENV_EXAMPLE.exists():
        print("ERROR: Missing Agentcy/.env.example")
        return 2
    ...
    print("\n2) Load env vars")
    print("   set -a && source Agentcy/.env && set +a")

    print("\n3) Verify Azure Foundry connection")
    print("   ./.venv/bin/python Agentcy/scripts/foundry_smoke_test.py")

    print("\n4) Raise a PR that adds hello-world.txt (dry-run first)")
    print(
        "   ./.venv/bin/python Agentcy/scripts/foundry_to_github_pr.py \\\n"
        "     --repo "
        + github_repo
        + " \\\n"
        "     --base main \\\n"
    ...
    print("- Env template: Agentcy/.env.example")
    print("- Connection test: Agentcy/scripts/foundry_smoke_test.py")
    print("- PR creator: Agentcy/scripts/foundry_to_github_pr.py")
    ...
    if should_write and _yes_no("Run the Foundry smoke test now?", default=True):
        ...
        print("\nRunning smoke test...")
        rc = subprocess.run(
            [sys.executable, "Agentcy/scripts/foundry_smoke_test.py"],
            cwd=str(REPO_ROOT),
        ...
        if _yes_no("Generate a PR proposal (dry-run) to add hello-world.txt?", default=True):
            ...
            cmd = [
                sys.executable,
                "Agentcy/scripts/foundry_to_github_pr.py",
                "--repo",
                github_repo,
                "--base",
                "main",
            ...
            if rc2 != 0:
                return int(rc2)
            print("\nDry-run complete. Inspect Agentcy/generated/pr_proposal.json")
