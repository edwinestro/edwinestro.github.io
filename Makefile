...
.venv/bin/python:
	...
	./.venv/bin/python -m pip install -r Agentcy/requirements.txt
	@if [ -f Agentcy/requirements-dev.txt ]; then ./.venv/bin/python -m pip install -r Agentcy/requirements-dev.txt; fi
...
copiping:
	$(MAKE) venv
	./.venv/bin/python Agentcy/scripts/setup_wizard.py

# Just run the Foundry smoke test (expects env vars already set)
copiping-smoke:
	$(MAKE) venv
	./.venv/bin/python Agentcy/scripts/foundry_smoke_test.py

# Run Foundry smoke test with an optional interactive prompt for PROJECT_API_KEY
copiping-connect:
	$(MAKE) venv
	./.venv/bin/python Agentcy/scripts/run_smoke_with_prompt.py
